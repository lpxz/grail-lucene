 ...
 */
public abstract class BenchmarkNoRefl {
  ...
  public BenchmarkNoRefl(Config config,  boolean silent) throws Exception {
    // TODO this is very ugly
    BenchmarkNoRefl.silent = silent;
   ...
   * 
   * @param logDir Scratch directory
   */
  public BenchmarkNoRefl(Config config) throws Exception {
    this(config,  true);
  }

  protected void initialize() throws Exception {
    System.setProperty("java.util.logging.config.file", fileIn(logDir, config.name + ".log"));
    synchronized (System.out) {
      if (out == null) {
        out = new TeePrintStream(System.out, new File(logDir, "stdout.log"));
        out.enableOutput(!silent);
      }
    }
    synchronized (System.err) {
      if (err == null) {
        err = new TeePrintStream(System.err, new File(logDir, "stderr.log"));
        err.enableOutput(!silent);
      }
    }
   
  // create the load originally
    prepare();
  }
  ...
  protected void prepare() throws Exception {
    unpackZipFileResource("dat/" + config.name + ".zip", logDir);
  }
  ...
  public boolean validate(String size) {
    ...
    for (String file : config.getOutputs(size)) {
      ...
      if (config.hasDigest(size, file)) {
        ...
        try {
          digest = Digest.toString(FileDigest.get(fileInScratch(file), config.isTextFile(size, file), config.filterScratch(size, file), logDir));
      ...
      if (config.hasLines(size, file)) {
        ...
        int lines;
        try {
          lines = lineCount(new File(logDir, file));
        } catch (FileNotFoundException e) {
      ...
      if (config.hasBytes(size, file)) {
        ...
        long bytes;
        try {
          bytes = byteCount(new File(logDir, file));
        } catch (FileNotFoundException e) {
          System.err.println("File not found, " + file);
          bytes = -1;
        } catch (IOException e) {
      ...
      if (config.checkExists(size, file)) {
        if (!new File(logDir, file).exists()) {
    ...
    for (String file : config.getOutputs(size)) {
      if (file.equals("$stdout") || file.equals("$stderr")) {
        ...
        if (!config.isKept(size, file))
          deleteFile(new File(logDir, file));
      }
    }
  }
  ...
  public void cleanup() {
    if (!preserve) {
      deleteTree(new File(logDir, config.name));
    }
  }
  ...
  public static URL getURL(String fn) {
    ClassLoader cl = BenchmarkNoRefl.class.getClassLoader();
  ...
  public static void setCommandLineOptions(CommandLineArgs line) {
    ...
    if (line.getValidationReport() != null)
      BenchmarkNoRefl.enableValidationReport(line.getValidationReport());
  }
